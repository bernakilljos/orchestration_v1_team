#!/bin/bash
# common.sh - design_web 공통 헬퍼 (dry-run·검증·로깅)

set -uo pipefail

design_web="design_web"
REPO_ROOT="${CLAUDE_PROJECT_ROOT:-$(pwd)}"
LOG_DIR="$REPO_ROOT/.claude/state/$design_web"
DATA_DIR="$REPO_ROOT/data/$design_web/$(date +%Y-%m-%d)"
mkdir -p "$LOG_DIR" "$DATA_DIR"

log_info() {
  local msg="$1"
  local ts=$(date -u +%Y-%m-%dT%H:%M:%SZ)
  printf '{"ts":"%s","level":"INFO","plugin":"%s","msg":"%s"}\n' "$ts" "$design_web" "$msg" >> "$LOG_DIR/log.jsonl"
  echo "[INFO] $msg"
}
